...
class GPS:
    ...
    def remove_duplicate_prompts(self, prompts: List[str]) -> List[str]:
        ...
        all_prompts = set()
        for i in range(config.iteration_t):
            G_t_so_far_file_name = f"output/deepseek-coder/G_t_for_iteration_{
                config.iteration_t}.txt"
            with open(G_t_so_far_file_name, "r") as f:
                G_t_so_far = f.readlines()
                for prompt in G_t_so_far:
                    all_prompts.add(prompt)
        unique_prompts = [p for p in prompts if p not in all_prompts]
    ...
    def _process_single_query(
        self, query: str, query_id: str, prompt_id: str, prompt: str
    ) -> Optional[int]:
    ...
    def _apply_augmentations(self, prompt: str, iteration: int) -> List[str]:
        ...
        # feedback enhanced augmentation
        try:
        ...
        # open security enhancement augmentation
        try:
    ...
    def optimize(self, dev_set: List[str]) -> List[Tuple[int, str]]:
        ...
        with open(config.G_t_for_iteration_prev_t_file, "r") as f:
            G_t = f.readlines()
        # G_t = self.initial_prompts
        stored_G = []
        reproductive_groups = []
        t = config.iteration_t
        ...
        self._log_G_t_for_iteration_x(G_t)
    ...
    def _log_G_t_for_iteration_x(self, G_t: List[str]):
        """Log information about the G_t for a given iteration."""
        with open(config.G_t_for_iteration_t_file, "a+") as f:
            for prompt in G_t:
                f.write(f"{prompt}\n")
