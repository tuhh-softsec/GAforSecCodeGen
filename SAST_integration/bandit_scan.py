...
from SAST_integration.abs_sast_scan import AbstractSastScan


class BanditScan(AbstractSastScan):
    """ class to scan code files one at a time, process the results and write to a file. """

    def __init__(self):
        super().__init__()

    def run_sast(self, filepath, query_id):
        ...
        bandit_output_filepath = os.path.join(
            output_dir, f"{query_id}-test.json")
        print(f"Bandit scanning in progress for file {filepath}......")
        cmd = f"bandit -r -f json {filepath} -o {bandit_output_filepath}"

        try:
            result = subprocess.run(
                # need to add check=True as well to the arugment. But later
